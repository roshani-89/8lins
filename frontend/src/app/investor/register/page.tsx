    ...
    <main className="min-h-screen bg-void flex items-center justify-center relative overflow-hidden">
      ...
      <div className="w-full max-w-[460px] px-6 py-10 relative z-10 animate-in fade-in zoom-in duration-700">
        <div className="text-center mb-10">
          <Link href="/" className="font-display text-5xl md:text-6xl tracking-[10px] text-navy inline-block mb-4 hover:scale-105 transition-transform duration-500">
            <em className="text-green not-italic">8</em>LINES
          </Link>
          <div className="text-[10px] tracking-[6px] text-green uppercase font-black uppercase">Investor Onboarding Protocol</div>
        </div>
        ...
        <div className="bg-white border border-navy/5 overflow-hidden shadow-2xl cut-lg">
          <div className="bg-navy/[0.02] border-b border-navy/5 px-8 py-5 flex items-center justify-between">
            ...
            <div className="flex gap-2"><div className="w-2.5 h-2.5 rounded-full bg-green/60 glow-green animate-pulse"/></div>
          </div>
          ...
          <div className="p-10">
            {step==='details' && (
              <div className="space-y-8">
                <div>
                  <label className="text-[9px] tracking-[4px] text-navy/30 uppercase block mb-3 font-bold ml-1">Full Legal Name</label>
                  <input value={form.name} onChange={e=>setForm(f=>({...f,name:e.target.value}))} placeholder="As per official documents" 
                    className="w-full bg-navy/5 border border-navy/10 px-6 py-4 text-[13px] text-navy font-mono placeholder:text-navy/20 focus:border-green/20 outline-none transition-all font-medium"/>
                </div>
                <div>
                  ...
                  <div className="flex gap-4">
                    ...
                    <input value={form.phone} onChange={e=>setForm(f=>({...f,phone:e.target.value.replace(/\D/g,'').slice(0,10)}))} placeholder="9XXXXXXXXX" 
                      className="flex-1 bg-navy/5 border border-navy/10 px-6 py-4 text-[14px] text-navy font-mono placeholder:text-navy/20 focus:border-green/20 outline-none transition-all font-medium"/>
                  </div>
                </div>
                <div>
                ...
                <button onClick={handleSend} disabled={loading} 
                  className="w-full bg-green text-white text-[11px] tracking-[5px] uppercase py-5 font-black cut-lg hover:shadow-glow hover:bg-green/90 transition-all disabled:opacity-50 mt-4">
            ...
            {step==='otp' && (
              <div className="space-y-8">
                <div>
                  <h1 className="font-display text-5xl text-navy mb-2 leading-[0.85] uppercase">VERIFY<br/><span className="text-green font-black">PROTOCOL.</span></h1>
                ...
                <div className="flex justify-center gap-3">
                  {otp.map((v,i)=>(
                    <input key={i} ref={el=>{refs.current[i]=el}} value={v} onChange={e=>handleChange(i,e.target.value)}
                      ...
                      className="w-12 h-16 bg-navy/5 border border-navy/10 text-center font-display text-3xl text-green outline-none focus:border-green/30 transition-all font-black"/>
                  ))}
                </div>
                
                <button onClick={handleVerify} disabled={loading||otp.join('').length!==6} 
                  className="w-full bg-green text-white text-[11px] tracking-[5px] uppercase py-5 font-black cut-lg transition-all hover:bg-green/90 hover:shadow-glow">
